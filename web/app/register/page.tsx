...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setPending(true);
    const { ok, data } = await apiFetch<{ errors?: string[] }>("/api/v1/auth/register", {
      method: "POST",
      body: JSON.stringify({
        email,
        password,
        account_name: accountName || "Workspace",
      }),
    });
    setPending(false);
    if (!ok) {
      const msg =
        data && typeof data === "object" && "errors" in data
          ? (data as { errors: string[] }).errors?.join(", ") || "Registration failed"
          : "Registration failed";
      setErr(msg);
      return;
    }
  ...
  return (
    <div className="bg-background relative flex min-h-screen flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm">
        ...
        <h1 className="text-on-surface text-center text-xl font-semibold tracking-tight">Create account</h1>
        <p className="text-on-surface-variant mt-1 text-center text-sm">Start building with NetIQ</p>
        <form onSubmit={onSubmit} className="mt-8 flex flex-col gap-4">
          <label className="block space-y-1">
            <span className="text-on-surface-variant text-xs">Workspace name</span>
            <input
              type="text"
              value={accountName}
          ...
          <label className="block space-y-1">
            ...
            <input
              ...
              className="border-outline-variant bg-surface-container-low text-on-surface focus:border-on-surface w-full rounded-md border px-3 py-2 text-sm outline-none"
            />
            <span className="text-on-surface-variant text-xs">At least 8 characters.</span>
          </label>
